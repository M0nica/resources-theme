...
${reservedFields.map(f => `  * "${f}"`).join(`\n`)}
  ...
  if (!internalPage.component.includes(`/.cache/`) && process.env.NODE_ENV === `production` && !fileOkCache[internalPage.component]) {
    ...
    if (!fileContent.includes(`export default`) && !fileContent.includes(`module.exports`) && !fileContent.includes(`exports.default`) && !fileContent.includes(`exports["default"]`) && !fileContent.match(/export \{[\s\S]* as default[\s\S]*\}/) && // this check only applies to js and ts, not mdx
...
const createNode = (node, plugin, actionOptions = {}) => {
  ...
  if (oldNode && !hasNodeChanged(node.id, node.internal.contentDigest)) {
    updateNodeAction = Object.assign({}, actionOptions, {
      plugin,
      type: `TOUCH_NODE`,
      payload: node.id
    });
