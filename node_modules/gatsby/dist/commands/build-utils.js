...
const checkAndRemoveEmptyDir = (publicDir, pagePath) => {
  const pageHtmlDirectory = path.dirname((0, _pageHtml.getPageHtmlFilePath)(publicDir, pagePath));
  const pageDataDirectory = path.join(publicDir, `page-data`, fixedPagePath(pagePath));
  const hasFiles = fs.readdirSync(pageHtmlDirectory); // if page's html folder is empty also remove matching page-data folder
...
const removePageFiles = ({
  ...
  const removePageData = pageKeys.map(pagePath => removePageDataFile({
    publicDir
  }, pagePath));
