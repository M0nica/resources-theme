...
.replace(/(^\/+|\/+$)/g, ``).split(`/`); // end of copied `@reach/router` internals
  ...
  if (matchPathPages.length) {
    ...
    pages.forEach((page, index) => {
      ...
      if (isInsideMatchPath) {
        newMatches.push(createMatchPathEntry(Object.assign({}, page, {
          matchPath: page.path
        }), index));
      }
...
\n`;
  asyncRequires += `exports.components = {\n${components.map(c => {
    ...
    const relativeComponentPath = path.relative(path.join(program.directory, `.cache`), c.component);
    return `  "${c.componentChunkName}": () => import("${slash(`./${relativeComponentPath}`)}" /* webpackChunkName: "${c.componentChunkName}" */)`;
  }).join(`,\n`)}
}\n\n`;
  ...
  await Promise.all([writeAndMove(`sync-requires.js`, syncRequires), writeAndMove(`async-requires.js`, asyncRequires), writeAndMove(`match-paths.json`, JSON.stringify(matchPaths, null, 4))]);
  return true;
};

const debouncedWriteAll = _.debounce(async () => {
